...
var debug = require('debug')('oe:studio');
...
module.exports = (app, name) => {
    ...
    function upsertBootData(path, data, uniqueField) {
        ...
        var dataIndex = bootData.findIndex(function getUniqueRecord(d) {
            return d && d[uniqueField] == data[uniqueField]
        });
        if (dataIndex == -1) {
            bootData.push(data);
        } else {
    ...
    function attachWorkFlowToModel() {
        fnGetModels(function (modelsList) {
            ...
            inquirer.prompt([{
                ...
                writeBootDataToModel('WorkflowManager', attachWorkflowDef, 'name');
                app.navigate('home');
            });
        })
    }
    ...
    switch (name) {
        case "studio:defaultUI": generateDefaultUI(); break;
        case "studio:createRoute": generateUIRoute(); break;
        case "studio:createNavigation": generateNavigationLink(); break;
        case "studio:attachWorkflow": attachWorkFlowToModel(); break;
        case "studio:attachRules": attachRuleToModel(); break;
    }
