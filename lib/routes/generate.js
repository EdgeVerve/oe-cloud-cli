...
var debug = require('debug')('oe:define');

var defineCommands = ["default-ui", "ui-route", "navigation-link"];
var attachCommands = ["workflow","business-rules"]
module.exports = (app, name) => {
    // console.log("app", app, "name", name);
    var self = this;
    debug("name: ", name);
    var primaryCmd = name[0];
    var cmd = name[1];

    if(primaryCmd == "define"){
        if (defineCommands.indexOf(cmd) === -1) {
            app.env.error(chalk.red( cmd + ' is not a valid define command.\n') + chalk.yellow('\nAvailable options are ' + defineCommands.join(', ') + '.\n'));
            return;
        }
    }else{
        if (attachCommands.indexOf(cmd) === -1) {
            app.env.error(chalk.red( cmd + ' is not a valid attach command.\n') + chalk.yellow('\nAvailable options are ' + attachCommands.join(', ') + '.\n'));
            return;
        }
    }
    ...
    switch (cmd) {
        ...
        case "ui-route": generateUIRoute(); break;
        case "navigation-link": generateNavigationLink(); break;
        case "workflow": attachWorkFlowToModel(); break;
        case "business-rules": attachRuleToModel(); break;
    }
